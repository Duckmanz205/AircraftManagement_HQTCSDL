...
namespace QuanLyMayBay.Controllers
{
    public class AdminController : Controller
    {
        ...
        // ===================================================================
        // HELPER METHOD: Tính doanh thu theo tháng sử dụng Stored Procedure
        // ===================================================================
        private decimal TinhDoanhThuThang(int thang, int nam)
        {
            var thangParam = new SqlParameter("@Thang", thang);
            var namParam = new SqlParameter("@Nam", nam);
            var tongDoanhThuParam = new SqlParameter
            {
            ...
            return (decimal)(tongDoanhThuParam.Value ?? 0m);
        }
        ...
        public ActionResult ThongKe(DateTime? startDate, DateTime? endDate, string searchQuery, int page = 1)
        {
            ...
            DateTime filterEndDate = endDate.Value.AddDays(1); // Không còn cần thiết nhưng giữ lại để tránh lỗi
            ...
            // ===================================================================
            // 3. THAY ĐỔI: Sử dụng bảng THONGKE_DOANHTHU thay vì LINQ phức tạp
            // Bảng này được Trigger tự động cập nhật khi có vé mới
            // ===================================================================
            var query = from tk in db.THONGKE_DOANHTHU
                        where tk.NGAY >= startDate.Value && tk.NGAY <= endDate.Value
                        select new ChiTietDoanhThuModel
                        {
                            MaChuyenBay = tk.MACB,
                            NgayBay = tk.NGAY,
                            SoVe = tk.SOLUONGVE ?? 0,
                            DoanhThu = tk.TONGDOANHTHU ?? 0,
                            GiaTrungBinh = tk.SOLUONGVE > 0 
                                ? (tk.TONGDOANHTHU ?? 0) / tk.SOLUONGVE.Value 
                                : 0
                        };
        ...
        private List<ChiTietDoanhThuModel> GetFilteredData(DateTime? startDate, DateTime? endDate, string searchQuery)
        {
            ...
            // ===================================================================
            // THAY ĐỔI: Sử dụng bảng THONGKE_DOANHTHU (giống method ThongKe)
            // ===================================================================
            var query = from tk in db.THONGKE_DOANHTHU
                        where tk.NGAY >= startDate.Value && tk.NGAY <= endDate.Value
                        select new ChiTietDoanhThuModel
        ...
        // ===================================================================
        // ACTION: Thống kê khách hàng theo quốc gia (Sử dụng SQL Function)
        // ===================================================================
        public ActionResult ThongKeQuocGia()
        {
            // Gọi Function SQL: fn_ThongKeKhachTheoQuocGia()
            var result = db.Database.SqlQuery<ThongKeQuocGiaViewModel>(
                "SELECT * FROM fn_ThongKeKhachTheoQuocGia()"
            ).ToList();

            // Trả về JSON để sử dụng trong AJAX hoặc Chart
